...
def fetch_layer_for_zone(apikey, layer_code, zone_data, cache_dir, delay=1.0):
    ...
    # Collect tiles per municipality (3km radius around each centroid)
    # Much more efficient than bounding box of entire zone
    all_tiles = set()
    for muni in zone_data["municipalities"]:
        if muni.get("lat") and muni.get("lon"):
            tiles = get_tiles_for_municipality(muni, zoom, buffer_km=3.0)
    ...
    for i, (x, y) in enumerate(sorted(all_tiles)):
        tile_file = layer_cache / f"{zoom}_{x}_{y}.json"
        if (i+1) % 50 == 0 or i == 0:
            print(f"    Progress: {i+1}/{len(all_tiles)}")
